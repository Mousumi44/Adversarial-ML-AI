...
class HTML_Malware():
    ...
    def preprocess(self, featureMask):
    ...
    def knn(self):
        ...
        knc.fit(self.X_train_prime, self.y_train)

        # Predict the class labels for the provided data
        pred_knc = knc.predict(self.X_test_prime)

        # Return probability estimates for the test data
        prob_knc = knc.predict_proba(self.X_test_prime)
    ...
    def svm_rbf(self):
        ...
        # Fit the model using X as training data and y as target values
        rbf_svc.fit(self.X_train_prime, self.y_train)

        return rbf_svc
        # Predict the class labels for the provided data
        pred_rbf_svc = rbf_svc.predict(self.X_test_prime)

        # Return probability estimates for the test data
        prob_rbf_svc = rbf_svc.predict_proba(self.X_test_prime)

        print(np.shape(np.array(self.X_train_prime))[1])
        for indx in range(len(prob_rbf_svc)):
            # for ele in range(np.shape(np.array(self.X_train_prime))[1]):
            #     print(ele)
            #
            #     print(self.X_train_prime[indx, ele]),
            print(np.array(self.X_train_prime)[indx, :])
            # print('\n')
            print("Label: %s, Predicted: %s, Probs: %s" % (self.y_test.to_numpy()[indx], pred_rbf_svc[indx], prob_rbf_svc[indx]))
    ...
    def svm_linear(self):
        ...
        # Fit the model using X as training data and y as target values
        linear_svc.fit(self.X_train_prime, self.y_train)

        # Predict the class labels for the provided data
        pred_linear_svc = linear_svc.predict(self.X_test_prime)

        # Return probability estimates for the test data
        prob_linear_svc = linear_svc.predict_proba(self.X_test_prime)
    ...
    def mlp(self):
        ...
        # Fit the model using X as training data and y as target values
        mlp.fit(self.X_train_prime, self.y_train)

        # Predict the class labels for the provided data
        pred_mlp = mlp.predict(self.X_test_prime)

        # Return probability estimates for the test data
        prob_mlp = mlp.predict_proba(self.X_test_prime)
