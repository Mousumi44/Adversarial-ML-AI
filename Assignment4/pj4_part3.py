...
class anIndividual:
    ...
    def calculate_fitness(self):
        ...
        self.data = np.array(html_obj.X_train_prime)
        ...
        if self.method == 'knn':
            ...
            try:
                ...
                if (math.pow((-1.0)*prob_ml[0][0]+(1.0)*prob_ml[0][1], 2)) < epsilon:
                    print('CI found')
                    ci.append(fv)
                    ...
                    for i in range(np.shape(self.data)[0]):
                        ed = self.euclideanDistance(self.data[i], fv)
                        if ed<minD:
                            minD = min(minD, ed)
                            ID = i
                    ...
                    ci_num += 1
                    print("Label: %s, Predicted: %s, Probs: %s, Evaluation Number: %s, CI Number: %s, Closest Instance ID: %s, Euclidean Distance: %s" % (
                        str((-1.0-self.L_1)*prob_ml[0][0]+(1.0+self.L1)*prob_ml[0][1]), pred_ml[0], prob_ml[0], evaluation_num, ci_num, ID, minD))
                    f.write("Label: %s, Predicted: %s, Probs: %s, Evaluation Number: %s, CI Number: %s, Closest Instance ID: %s, Euclidean Distance: %s\n" % (
                        str((-1.0-self.L_1)*prob_ml[0][0]+(1.0+self.L1)*prob_ml[0][1]), pred_ml[0], prob_ml[0], evaluation_num, ci_num, ID, minD))
